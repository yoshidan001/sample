 ...
 * 
 * かたやん印刷所の所長かたやんは、使った活字を元の場所に戻さないので、 いつも活字を入れているボックスはぐちゃぐちゃで、
 * ほしい活字をすぐに見つけ出すことができません。
 * 
 * そこで見かねた印刷所の副所長エミーは、活字を入れているボックスビデオで撮影し、 画像解析し、ボックスのどこにどの活字があるかを解析し、
 * ほしい文字列の活字がどこにあるか瞬時にみつけだすプログラミングを作ることにしました。
 * 
 * 現状の状態のボックスは以下のようになっています。
 * 
 * 
 * 6×6のボックスの中から"PRO, PROGRAMMER"という活字を探し出したいです。 カンマやスペースは1つの活字としてカウントします。
 * ですので、今回は文字13個、カンマ1個、スペース1個の合計15個の活字が必要です。 一度使った活字は2回使うことはできません。
 * 
 * 活字の場所は、(縦番号,横番号)で表すと以下のような順番で活字を取り出すことができます。
 * 縦番号は上から下の方向に、横番号は左から右の方向に、1から数えます。 1,4 = P 2,1 = R 1,6 = O 5,2 = , 5,4 =
 * space 2,2 = P 3,4 = R 4,5 = O 5,1 = G 4,3 = R 1,1 = A 2,5 = M 6,1 = M 2,4 = E
 * 6,6 = R
 * 
 * 
 ...
 * 
 * 画像解析したボックスのデータはCSVで出力されます。 ボックスのデータを使ってほしい文字列を作るための活字の在り処を探しだしてください。
 * 
 * ■資料 gutenberg_Java.zip zipファイルを解凍すると以下のファイルが入っています。
 * 
 * gutenberg.csv このボックスから"STAY HUNGRY, STAY FOOLISH"という活字を取り出してください。
 * 文字21個、カンマ1個、スペース3個で、合計25個の活字が必要です。
 * 
 * answer.txt 解答用テキストファイルです。
 * 
 * ■解答方法 解答用テキストファイルに書かれていることに従ってファイルを作成し、 完成したらファイルアップロードして提出ください。
 */
